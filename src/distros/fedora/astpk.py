...
def prepare(snapshot):
    ...
    # pacman gets weird when chroot directory is not a mountpoint, so the following mount is necessary ### IS THE FOLLOWING LINE NEEDED?
...
def switchtmp():
    ...
    if "tmp0" in mount:
        ...
        os.system(f"sed -i 's,@.snapshots{distro_suffix}/rootfs/snapshot-tmp0,@.snapshots{distro_suffix}/rootfs/snapshot-tmp,g' /.snapshots/rootfs/snapshot-tmp/boot/loader/entries/current.cfg")
        ...
        os.system(f"sed -i 's,@.snapshots{distro_suffix}/rootfs/snapshot-tmp,@.snapshots{distro_suffix}/rootfs/snapshot-tmp0,g' /etc/mnt/boot/loader/entries/current.cfg")
        os.system(f"sed -i 's,@.snapshots{distro_suffix}/rootfs/snapshot-tmp,@.snapshots{distro_suffix}/rootfs/snapshot-tmp0,g' /.snapshots/rootfs/snapshot-tmp0/boot/loader/entries/current.cfg")
    ...
    if "Fedora Linux" in gconf:
        gconf = re.sub('snapshot \d', '', gconf)
        gconf = gconf.replace(f"Fedora Linux", f"Fedora Linux last booted deployment (snapshot {snap})")
    grubconf.close()
    #os.system("sed -i '$ d' /etc/mnt/boot/loader/entries/current.cfg")
    grubconf = open("/etc/mnt/boot/loader/entries/current.cfg", "a")
    grubconf.write(gconf)
    grubconf.write("\n")
    #grubconf.write("### END /etc/grub.d/41_custom ###")
    grubconf.close()

    grubconf = open("/.snapshots/rootfs/snapshot-tmp0/boot/loader/entries/current.cfg","r")
    line = grubconf.readline()
    while "BEGIN /etc/grub.d/10_linux" not in line:
        line = grubconf.readline()
    line = grubconf.readline()
    ...
    if "Fedora Linux" in gconf:
        ...
        gconf = gconf.replace(f"Fedora Linux", f"Fedora Linux last booted deployment (snapshot {snap})")
    grubconf.close()
    #os.system("sed -i '$ d' /.snapshots/rootfs/snapshot-tmp0/boot/grub2/grub.cfg")
    grubconf = open("/.snapshots/rootfs/snapshot-tmp0/boot/loader/entries/current.cfg", "a")
    grubconf.write(gconf)
    grubconf.write("\n")
    #grubconf.write("### END /etc/grub.d/41_custom ###")
    grubconf.close()
...
def main(args):
    ...
    if isChroot == True and ("--chroot" not in args):
        ...
        ast_help()
    elif arg == "ast-sync" and (lock != True): # Currently this lock is ignored ###REVIEW_LATER
        ast_lock()
        ast_sync()
        ast_unlock()
