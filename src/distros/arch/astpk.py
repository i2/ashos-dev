            ...
            with open('/boot/efi/EFI/map.txt', 'r') as f:
                ...
                for row in input_file:
                    if row["DISTRO"] == next_distro:
                        ...
                        bo = subprocess.check_output("efibootmgr | grep BootOrder | awk '{print $1}'", shell=True).decode('utf-8').strip()
                        temp = bo.replace(f'{row["BootOrder"]},', "")
                        newbo = f"{row['BootOrder']},{temp}"
                        os.system(f'efibootmgr --bootorder {newbo}')
            break
        else:
