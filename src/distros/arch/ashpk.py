...
def install_profile(snapshot, profile, path="", dist=get_distro_suffix()):
    if not os.path.isfile(f"$HOME/ashos/src/profiles/{profile}/packages{dist}.txt") and path == "":
        ...
        install(snapshot, subprocess.check_output(f"cat {path}/src/profiles/{profile}/packages{dist}.txt | grep -E -v '^#|^$'", shell=True).decode('utf-8').replace('\n', ' '))
    elif os.path.isfile(f"$HOME/ashos/src/profiles/{profile}/packages{dist}.txt"):
        install(snapshot, subprocess.check_output(f"cat $HOME/ashos/src/profiles/{profile}/packages{dist}.txt | grep -E -v '^#|^$'", shell=True).decode('utf-8').replace('\n', ' '))
    else:
