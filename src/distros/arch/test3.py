...
def install_profile(snapshot, profile, path="", ds=get_distro_suffix()):
    if not os.path.isfile(f"$HOME/ashos/src/profiles/{profile}/packages{ds}.txt") and path == "":
        os.system(f"curl -o /tmp/ -LO https://github.com/i2/ashos-dev/blob/debian/src/profiles/{profile}/packages{ds}.txt")
        install(snapshot, subprocess.check_output(f"cat /tmp/packages{ds}.txt | grep -E -v '^#|^$'", shell=True).decode('utf-8').replace('\n', ' '))
    elif path != "":
        install(snapshot, subprocess.check_output(f"cat {path}/src/profiles/{profile}/packages{ds}.txt | grep -E -v '^#|^$'", shell=True).decode('utf-8').replace('\n', ' '))
    elif os.path.isfile(f"$HOME/ashos/src/profiles/{profile}/packages{ds}.txt"):
        install(snapshot, subprocess.check_output(f"cat $HOME/ashos/src/profiles/{profile}/packages{ds}.txt | grep -E -v '^#|^$'", shell=True).decode('utf-8').replace('\n', ' '))
    else:
        print("F: Failed to install from the profile.")
...
def switchtmp():
    ...
    mount = get_tmp()
    part = get_part()
    os.system("mkdir -p /etc/mnt/boot >/dev/null 2>&1")
    ...
    if "deploy-aux" in mount:
        ...
        sfile = open("/.snapshots/rootfs/snapshot-deploy-aux/usr/share/ash/snap", "r")
        snap = sfile.readline()
        snap = snap.replace(" ", "")
        sfile.close()
    else:
        ...
        sfile = open("/.snapshots/rootfs/snapshot-deploy/usr/share/ash/snap", "r")
        snap = sfile.readline()
        snap = snap.replace(" ", "")
        sfile.close()
    #
    ...
    if "Arch Linux" in gconf:
        ...
        gconf = gconf.replace("Arch Linux", f"Arch Linux last booted deployment (snapshot {snap})")
    grubconf.close()
...
def ash_update():
    cdir = os.getcwd()
    os.chdir("/tmp")
    excode = str(os.system("curl -O 'https://raw.githubusercontent.com/ashos/ashos/main/ashpk.py'"))
    if int(excode) == 0:
...
def main(args):
    ...
    if isChroot == True and ("--chroot" not in args):
        ...
        ash_unlock()
    elif arg == "install-profile" and (lock != True):
        ash_lock()
        ...
        ash_help()
    elif arg == "upself" and (lock != True): # Currently this lock is ignored ###REVIEW_LATER
        ash_lock()
        ash_update()
        ash_unlock()
...
if __name__ == "__main__":
    ...
    import anytree
    args = list(sys.argv)
###    distro = subprocess.check_output(['sh', '/usr/bin/detect_os.sh']).decode('utf-8').replace('"', "").strip() ### MOVED TO get_distro_suffix function
    main(args)
