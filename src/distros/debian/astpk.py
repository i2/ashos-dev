...
def update_base():
    ...
    if os.path.exists(f"/.snapshots/rootfs/snapshot-chr{snapshot}"):
        ...
        prepare(snapshot)
        excode = str(os.system(f"chroot /.snapshots/rootfs/snapshot-chr{snapshot} apt-get update"))
        if int(excode) == 0:
            posttrans(snapshot)
...
def posttrans(snapshot):
    ...
    os.system(f"cp -r -n --reflink=auto /.snapshots/rootfs/snapshot-chr{snapshot}/var/cache/apt/* /var/cache/apt/ >/dev/null 2>&1") ### REVIEW_LATER IS THIS NEEDED?
